...
# Diagrams from FundEd_Project_Diagrams.md
diagrams = [
    ("1. Activity Diagram", """
graph TD
...
def render_mermaid(code):
    ...
    base64_string = base64.urlsafe_b64encode(json_str.encode('utf-8')).decode('ascii').strip('=')
...
def generate_pdf():
    ...
    for title, code in diagrams:
        ...
        url = render_mermaid(code)
        
        # Wide diagrams benefit from Landscape (L)
        is_wide = any(keyword in title for keyword in ["Activity", "ER", "Data Flow", "Sequence", "Architecture", "Use Case"])
        orientation = 'L' if is_wide else 'P'
        
        try:
            response = requests.get(url, timeout=30)
            if response.status_code == 200:
                temp_img = f"temp_{title.replace(' ', '_').replace(':', '')}.png"
                ...
                # Define printable area (with 10mm margins)
                # Page width/height: P=210x297, L=297x210
                page_w = 297 if orientation == 'L' else 210
                page_h = 210 if orientation == 'L' else 297
                
                margin = 10
                max_w = page_w - (2 * margin)
                max_h = page_h - 40 # Account for title and bottom margin
                ...
                # Center horizontally and vertically (roughly)
                x_pos = (page_w - final_w) / 2
                y_pos = 25 # Start after title
                
                pdf.image(temp_img, x=x_pos, y=y_pos, w=final_w, h=final_h)
                
                os.remove(temp_img)
            else:
                print(f"Failed to render {title}: {response.status_code}")
                pdf.add_page(orientation=orientation)
                pdf.set_font("Arial", 'B', 16)
                pdf.cell(0, 10, f"Failed to render (HTTP {response.status_code}): {title}", ln=True, align='C')
        except Exception as e:
            print(f"Error for {title}: {e}")
            pdf.add_page(orientation=orientation)
            pdf.set_font("Arial", 'B', 16)
            pdf.cell(0, 10, f"Error: {title}", ln=True, align='C')

    pdf.output("FundEd_Project_Diagrams.pdf")
    print("PDF generated: FundEd_Project_Diagrams.pdf")
